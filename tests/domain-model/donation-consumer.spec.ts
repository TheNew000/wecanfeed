...
describe('DonationConsumer Model and Schema', () => {
  ...
  it('should define the averageOccupancy property model', () => {
    expect(averageOccupancy.type).to.equal(Number, 'Expected type to equal Number');
    expect(averageOccupancy.required).to.equal(false, 'Expected required to equal false');
    expect(averageOccupancy.unique).to.equal(false, 'Expected unique to equal false');
  });

  it('should define the isAccepting property model', () => {
    expect(isAccepting.type).to.equal(Boolean, 'Expected type to equal Boolean');
    expect(isAccepting.required).to.equal(true, 'Expected required to equal true');
    expect(isAccepting.unique).to.equal(false, 'Expected unique to equal false');
  });
